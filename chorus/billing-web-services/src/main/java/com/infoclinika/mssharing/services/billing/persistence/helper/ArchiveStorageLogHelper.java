...
public class ArchiveStorageLogHelper extends AbstractStorageLogHelper {
    ...
    public ArchiveStorageLogHelper(HourlyArchiveStorageUsageRepository hourlyUsageRepository,
        ...
        this.hourlyMaxThreads = billingPropertiesProvider.getHourlyMaxArchiveThreads();
        this.dailyMaxThreads = billingPropertiesProvider.getDailyMaxArchiveThreads();
    }
    ...
    protected Function<Lab, Page<ChargeableItemUsage>> sumLogsFn(final long dayToLog, final Pageable pageable) {
        ...
        return input -> {
            ...
            final Page<GroupedStorageUsage<HourlyArchiveStorageUsage>> storageUsagePage =
                hourlyUsageRepository.groupedNotCompressedUsagesByFile(input.getId(), dayToLog, pageable);
            return new PageImpl<>(
                StreamSupport.stream(storageUsagePage.spliterator(), false).map(sumDailyUsageFn(dayToLog)::apply)
                    .collect(Collectors.toList()), pageable, storageUsagePage.getTotalElements());

        };
    }

    private Function<GroupedStorageUsage<HourlyArchiveStorageUsage>, ChargeableItemUsage> sumDailyUsageFn(
        final long dayToLog) {
        return input -> {
    ...
    public void log(final long logInterval) {
        ...
        for (Map.Entry<String, S3ObjectSummary> summaryEntry : files.entrySet()) {
            ...
            if ((counter % filesLogOperationCount == 0) || (counter == (totalFilesSize))) {
                ...
                final Set<Callable<Iterable<HourlyArchiveStorageUsage>>> tasks =
                    buildArchiveUsageTasks(logInterval, filesMap);

                final Date filesLoggedTime = new Date();

                logger.debug("Executing archive usage tasks... Files: " + counter + " of " + totalFilesSize);
                final List<Future<Iterable<HourlyArchiveStorageUsage>>> processedTasks =
    ...
    private Set<Callable<Iterable<HourlyArchiveStorageUsage>>> buildArchiveUsageTasks(final long logInterval,
                                                                                      final Map<String,
                                                                                          S3ObjectSummary> files) {

        final List<BillingFileView> allByArchiveId =
            files.keySet().isEmpty() ? Collections.emptyList() :
                fileViewRepository.findAllByArchiveId(files.keySet());
    ...
    private Optional<HourlyArchiveStorageUsage> logMetaFile(BillingFileView fileMetaData, S3ObjectSummary file,
                                                            long logInterval) {

        final Date now = new Date();

        if (fileMetaData == null) {
            logger.error("Cannot find archived file in database. Bucket: '" + file.getBucketName() + "',  Key: '" +
                file.getKey() + "'");
    ...
    private HourlyArchiveStorageUsage newArchiveStorageUsage(Long lab, BillingFileView file, Date now,
                                                             S3ObjectSummary s3ObjectSummary) {

        final long amount = 0;

        final HourlyArchiveStorageUsage usage =
            new HourlyArchiveStorageUsage(lab, file.getOwner(), file.getId(), s3ObjectSummary.getSize(),
