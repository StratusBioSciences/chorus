...
public interface ArchiveStorageUsageRepository<T extends ArchiveStorageUsage> extends StorageFeatureUsageRepository<T> {
     ...
     */
    String SUM_PRICE =
        "sum(round(s.charge/com.infoclinika.mssharing.services.billing.persistence.enity.ChargeableItemUsage" +
            ".PRICE_SCALE_VALUE, com.infoclinika.mssharing.services.billing.persistence.enity.ChargeableItemUsage" +
            ".PRICE_PRECISION))";
    ...
    @Override
    @Query(
        "select new com.infoclinika.mssharing.services.billing.persistence.enity.storage.ArchiveStorageUsage(s.lab, " +
            "s.user,s.file, s.bytes, s.timestamp, s.usedBy, s.instrument," +
    ...
    Page<GroupedStorageUsage<T>> groupedNotCompressedUsagesByFile(@Param("lab") long lab, @Param("day") long dayToLog,
                                                                  Pageable pageable);
    ...
    @Query("select new com.infoclinika.mssharing.model.internal.repository.FeatureUsageByUser(" +
        " round(coalesce(" + SUM_PRICE + ", 0)), " +
        " s.user,  s.usedBy, count(distinct s.file)) from #{#entityName} s WHERE s.lab = :lab AND s.timestamp>:start " +
        "AND s.timestamp <= :end group by s.user")
    List<FeatureUsageByUser> groupUsagesByUser(@Param("lab") long lab,
    ...
    @Override
    @Query(
        "SELECT count(DISTINCT s.file) FROM #{#entityName} s where s.lab = :lab AND s.timestamp > :dateFrom " +
            "AND s.timestamp <= :dateTo")
