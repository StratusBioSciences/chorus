...
public interface DailyArchiveStorageUsageRepository
    ...
    @Query(
        value = "select new com.infoclinika.mssharing.services.billing.persistence.enity.dto.GroupedStorageUsage(ss, " +
            "  (select coalesce(sum(s.hours),0) from DailyArchiveStorageUsage s " +
            " where s.day=ss.day AND s.file = ss.file and compressed = false)," +
            "  (select coalesce(SUM(s.charge),0) from DailyArchiveStorageUsage s " +
            " where s.day=ss.day AND s.file = ss.file AND compressed = false))" +
            " from DailyArchiveStorageUsage ss where (ss.lab = :lab AND ss.day=:day) AND ss.compressed = false " +
            " AND ss.timestamp = (SELECT max(s.timestamp) FROM DailyArchiveStorageUsage s " +
    ...
    Page<GroupedStorageUsage<DailyArchiveStorageUsage>> groupedNotCompressedUsagesByFile(@Param("lab") long lab,
                                                                                         @Param("day") long dayToLog,
                                                                                         Pageable pageable);

    @Query(
        "select sum(u.bytes) from DailyArchiveStorageUsage u where u.lab = :lab " +
            "and u.day >= :fromDay and u.day <= :toDay group by u.day")
