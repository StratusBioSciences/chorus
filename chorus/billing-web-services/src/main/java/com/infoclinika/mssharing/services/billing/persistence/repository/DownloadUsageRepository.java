 ...
 */
public interface DownloadUsageRepository
    extends FeatureUsageRepository<DownloadUsage>, CrudRepository<DownloadUsage, Long> {
    /**
    ...
    @Query("select new com.infoclinika.mssharing.model.internal.repository.FeatureUsageByUser(" +
        ...
        " u.user,  u.usedBy, count(distinct u.file)) from DownloadUsage u WHERE (u.lab = :lab AND u.day=:day) group " +
        "by u.user")
    ...
    @Query("select new com.infoclinika.mssharing.model.internal.repository.FeatureUsageByUser(" +
        " round(coalesce(" + SUM_PRICE + ", 0)), " +
        " u.user,  u.usedBy, count(distinct u.file)) from DownloadUsage u WHERE u.lab = :lab AND u.timestamp>:start " +
        "AND u.timestamp <= :end group by u.user")
    ...
    List<Integer> groupNotPayedUsersFilesPricesByLab(@Param("lab") long lab,
                                                     @Param("lastPayedTimestamp") long lastPayedTimestamp);
    ...
    @Override
    @Query("SELECT count(DISTINCT s.file) FROM DownloadUsage s where s.lab = :lab AND s.timestamp > :dateFrom " +
            "AND s.timestamp <= :dateTo")
