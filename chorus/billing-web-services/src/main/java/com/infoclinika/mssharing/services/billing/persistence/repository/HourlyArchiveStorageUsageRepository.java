...
public interface HourlyArchiveStorageUsageRepository
    ...
    @Query(
        value = "select new com.infoclinika.mssharing.services.billing.persistence.enity.dto.GroupedStorageUsage(ss, " +
            " (select coalesce(sum(s.hours),0) from HourlyArchiveStorageUsage s where s.day=ss.day " +
            "AND s.file = ss.file and compressed = false)," +
            " (select coalesce(SUM(s.charge),0) from HourlyArchiveStorageUsage s where s.day=ss.day " +
            "AND s.file = ss.file AND compressed = false))" +
            " from HourlyArchiveStorageUsage ss where (ss.lab = :lab AND ss.day=:day) AND ss.compressed = false " +
            "AND ss.timestamp = (SELECT max(s.timestamp) FROM HourlyArchiveStorageUsage s WHERE s.day=ss.day " +
            "AND s.file = ss.file)",
    ...
    Page<GroupedStorageUsage<HourlyArchiveStorageUsage>> groupedNotCompressedUsagesByFile(@Param("lab") long lab,
