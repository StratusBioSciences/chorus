 ...
 */
public interface PublicDownloadUsageRepository
    extends FeatureUsageRepository<PublicDownloadUsage>, CrudRepository<PublicDownloadUsage, Long> {
    /**
     * sum(round(price/100000, 16))
     */
    String SUM_PRICE =
        "sum(round(u.charge/com.infoclinika.mssharing.services.billing.persistence.enity.ChargeableItemUsage" +
    ...
    @Override
    @Query(
        "select new com.infoclinika.mssharing.services.billing.persistence.enity.PublicDownloadUsage(s.lab, s.user," +
            "s.file, sum(s.bytes), " +
    ...
    @Query("select new com.infoclinika.mssharing.model.internal.repository.FeatureUsageByUser(" +
        ...
        " coalesce(u.user, 0),  u.usedBy, count(distinct u.file)) from PublicDownloadUsage u WHERE (u.lab = :lab " +
        "AND u.day=:day) group by u.user")
    List<FeatureUsageByUser> groupUsagesByUser(@Param("lab") long lab, @Param("day") long day);

    @Override
    @Query("select new com.infoclinika.mssharing.model.internal.repository.FeatureUsageByUser(" +
        " round(coalesce(" + SUM_PRICE + ", 0)), " +
        " u.user,  u.usedBy, count(distinct u.file)) from PublicDownloadUsage u WHERE u.lab = :lab " +
        "AND u.timestamp>:start AND u.timestamp <= :end group by u.user")
    ...
    @Query("SELECT count(DISTINCT s.file) FROM PublicDownloadUsage s where s.lab = :lab AND s.timestamp > :dateFrom " +
        "AND s.timestamp <= :dateTo")
