...
import com.infoclinika.mssharing.model.PaginationItems.AdvancedFilterQueryParams.AdvancedFilterPredicateItem.AdvancedFilterOperator;
...
public class BillingServiceImpl implements BillingService {
    ...
    public void logDownloadUsage(long actor, long file, long lab) {
        LOG.debug("Billing Service: download usage, actor: {}, file: {}, lab: {}", actor, file, lab);
        paymentManagement.logDownloadUsage(actor, file, lab);
        LOG.debug("Billing Service: completed action - download usage, actor: {}, file: {}, lab: {}", actor, file, lab);
    }

    @Override
    public void logPublicDownload(@Nullable Long actor, long file) {
        LOG.debug("Billing Service: log public download, actor:  {}, file: {}", actor, file);
        paymentManagement.logPublicDownload(actor, file);
        LOG.debug("Billing Service: completed action - log public download, actor:  {}, file: {}", actor, file);
    }
    ...
    public Invoice readInvoiceShortItem(long actor, long lab, long from, long to) {
        ...
        final ChargeableItemUsageReader.Invoice invoice =
            chargeableItemUsageReader.readInvoiceShortItem(actor, lab, new Date(from), new Date(to));
    ...
    public Collection<InvoiceLabLine> readLabsForUser(long actor) {
        ...
        final Set<InvoiceLabLine> result = chargeableItemUsageReader.readLabsForUser(actor).stream()
            .map(ChargeableItemUsageReader.InvoiceLabLine.WS_TRANSFORM::apply).collect(Collectors.toSet());
        LOG.debug("Billing Service: completed action - read labs for user: {}", actor);
        return result;
    }

    @Override
    public HistoryForMonthReference readMonthReference(long actor, long lab, long month) {
        LOG.debug("Billing Service: read read months references: {}", actor);
        final HistoryForMonthReference historyForMonth =
            chargeableItemUsageReader.readMonthReference(actor, lab, new Date(month));
    ...
    public LabInvoiceDetails readLabDetails(long actor, long lab) {
        ...
        final ChargeableItemUsageReader.LabInvoiceDetails labInvoiceDetails =
            chargeableItemUsageReader.readLabDetails(actor, lab);
        final LabInvoiceDetails result =
            ChargeableItemUsageReader.LabInvoiceDetails.WS_TRANSFORM.apply(labInvoiceDetails);
    ...
    public PagedItemInfo.PagedItem<InvoiceLabLine> readPagedAllLabs(ReadPagedAllLabsRequest request) {
        ...
        final Optional<PaginationItems.AdvancedFilterQueryParams> advancedFilterQueryParams =
            getAdvancedFilterQueryParamsOptional(request.wsPpagedItemInfo);
        PaginationItems.PagedItemInfo pagedItemInfo =
            new PaginationItems.PagedItemInfo(request.wsPpagedItemInfo.items, request.wsPpagedItemInfo.page,
        ...
        final PaginationItems.PagedItem<ChargeableItemUsageReader.InvoiceLabLine> invoice =
            chargeableItemUsageReader.readPagedAllLabs(request.actor, pagedItemInfo);
        final PagedItemInfo.PagedItem<InvoiceLabLine> result =
            new PagedItemInfo.PagedItem<>(invoice.totalPages, invoice.itemsCount, invoice.pageNumber, invoice.pageSize,
    ...
    public BillingFeatureItem readFeatureInfo(long feature) {
        ...
        final com.infoclinika.mssharing.model.helper.BillingFeatureItem billingFeatureItem =
            chargeableItemUsageReader.readFeatureInfo(feature);
        final BillingFeatureItem result =
            com.infoclinika.mssharing.model.helper.BillingFeatureItem.WS_TRANSFORM.apply(billingFeatureItem);
    ...
    public List<BillingFeatureItem> readFeatures() {
        ...
        final List<com.infoclinika.mssharing.model.helper.BillingFeatureItem> features =
            chargeableItemUsageReader.readFeatures();
        final List<BillingFeatureItem> transformedFeatures =
            features.stream().map(com.infoclinika.mssharing.model.helper.BillingFeatureItem.WS_TRANSFORM::apply)
    ...
    public long calculateRoundedPriceByUnscaled(long actualBalance, long unscaledValue) {
        LOG.debug(
            "Billing Service: calculate rounded price by unscaled: {actualBalance: {}, unscaledValue {} }",
            actualBalance, unscaledValue
        );
        final long result = paymentCalculationsHelper.calculateRoundedPriceByUnscaled(actualBalance, unscaledValue);
        LOG.debug(
            "Billing Service: completed action - calculate rounded price by unscaled: {actualBalance: {}, " +
                "unscaledValue {} }",
            actualBalance, unscaledValue
        );
        return result;
    ...
    public long calculateTotalToPayForLabForDay(long lab, long day) {
        ...
        LOG.debug("Billing Service: calculate total to pay for lab for day: { lab: {}, day: {} }", lab, dayDate);
        final long result = paymentCalculationsHelper.calculateTotalToPayForLabForDay(lab, dayDate);
        LOG.debug(
            "Billing Service: completed action - calculate total to pay for lab for day: { lab: {}, day: {} }", lab,
            dayDate
        );
        return result;
    }
    ...
    public Long calculateStoreBalanceForDay(long lab, long day) {
        final Date dayDate = new Date(day);
        LOG.debug("Billing Service: calculate store balance for day: { lab: {}, day: {} }", lab, dayDate);
        final Long result = paymentCalculationsHelper.calculateStoreBalanceForDay(lab, dayDate).orNull();
        LOG.debug(
            "Billing Service: completed action - calculate store balance for day: { lab: {}, day: {} }", lab, dayDate);
    ...
    public long calculateTotalToPayForLab(long lab, long from, long to) {
        ...
        LOG.debug(
            "Billing Service: calculate total to pay for lab:{ lab: {}, from: {}, to: {} }", lab, fromDate, toDate);
        final long result = paymentCalculationsHelper.calculateTotalToPayForLab(lab, fromDate, toDate);
        LOG.debug(
            "Billing Service: completed action - calculate total to pay for lab:{ lab: {}, from: {}, to: {} }", lab,
            fromDate, toDate
        );
        return result;
    }
    ...
    public Long calculateStoreBalance(long lab, long current, long nextDay) {
        ...
        LOG.debug(
            "Billing Service: calculate store balance: { lab: {}, current: {}, nextDay: {} }", lab, currentDate,
            nextDayDate
        );
        final Long result = paymentCalculationsHelper.calculateStoreBalance(lab, currentDate, nextDayDate).orNull();
        LOG.debug(
            "Billing Service: completed action - calculate store balance: { lab: {}, current: {}, nextDay: {} }", lab,
    ...
    public void logLabBecomeEnterprise(long actor, long lab, long time) {
        LOG.debug("Billing Service: lab enterprise, actor: {}, lab: {}, timestamp: {}", actor, lab, time);
        paymentManagement.logLabBecomeEnterprise(actor, lab, time);
        LOG.debug(
            "Billing Service: completed action - lab enterprise, actor: {}, lab: {}, timestamp: {}", actor, lab, time);
    }

    @Override
    public void logLabBecomeFree(@FormParam("actor") long actor, @FormParam("lab") long lab,
                                 @FormParam("time") long time) {
        LOG.debug("Billing Service: lab free, actor: {}, lab: {}, timestamp: {}", actor, lab, time);
        paymentManagement.logLabBecomeFree(actor, lab, time);
        LOG.debug("Billing Service: completed action - lab free, actor: {}, lab: {}, timestamp: {}", actor, lab, time);
    }

    @Override
    public void storeCreditForLab(long admin, long lab, long amount) {
        LOG.debug("Billing Service: store log credit, actor: {}, lab: {}, timestamp: {}", admin, lab, amount);
        paymentManagement.depositStoreCredit(admin, lab, amount);
        LOG.debug(
            "Billing Service: completed action - store log credit, actor: {}, lab: {}, timestamp: {}", admin, lab,
            amount
        );
    }
    ...
    public List<PendingCharge> getPendingChargesForLab(long actor, long lab, long timestamp) {
        final List<ChargeableItemUsageReader.MonthlyCharge> monthlyCharges =
            chargeableItemUsageReader.readMonthlyCharges(actor, lab, new Date(timestamp));
    ...
    public StorageUsage readStorageUsage(long actor, long lab) {
        final ChargeableItemUsageReader.StorageUsage storageUsage =
            chargeableItemUsageReader.readStorageUsage(actor, lab);
    ...
    private static Optional<PaginationItems.AdvancedFilterQueryParams> getAdvancedFilterQueryParamsOptional(
        PagedItemInfo wsPpagedItemInfo) {
        final PagedItemInfo.AdvancedFilterQueryParams af = wsPpagedItemInfo.advancedFilter;
        final PaginationItems.AdvancedFilterQueryParams advancedFilterQueryParams = af == null ? null :
            new PaginationItems.AdvancedFilterQueryParams(af.conjunction, from(af.predicates).transform(
                input -> {
                    final AdvancedFilterOperator operator = AdvancedFilterOperator.valueOf(input.operator.name());
                    final AdvancedFilterPredicateItem result =
                        new AdvancedFilterPredicateItem(input.prop, input.value, operator);
                    return result;
                }).toList());
