...
public class DetailsReaderImpl
    extends DefaultDetailsReader<ActiveFileMetaData, ActiveProject, ActiveExperiment, Instrument, Lab, Group,
    ...
    @Override
    protected ExperimentItem afterReadExperiment(final long actor,
                                                 SingleResultBuilder<ActiveExperiment, ExperimentItem> resultBuilder) {
        ...
        return resultBuilder.transform(new Function<ActiveExperiment, ExperimentItem>() {
            ...
            public ExperimentItem apply(ActiveExperiment experiment) {
                ...
                for (ExperimentSample experimentSample : samples) {
                    factorValues[sampleCounter] = experimentSample.getFactorValues()
                        .toArray(new String[experimentSample.getFactorValues().size()]);
                    sampleCounter++;
                }
                final List<String> samplesInOrder =
                    newArrayList(Collections2.transform(samples, new Function<ExperimentSample, String>() {
                        @Override
                ...
                final boolean isNgsExperiment = experiment.getInstrumentRestriction()
                    .getInstrumentModel()
                    .getStudyType()
                    .getName()
                    .equals(InstrumentsDefaults.NG_TECHNOLOGY_TYPE);
                final NgsRelatedExperimentInfo ngsRelatedInfo = ngsRelatedData != null && isNgsExperiment ?
                    new NgsRelatedExperimentInfo(
                        ngsRelatedData.getNgsExperimentTypeId(),
                        ngsRelatedData.isMultiplexing(),
                        samples.size(),
                        ngsRelatedData.getPairedEnd(),
                        ngsRelatedData.getExperimentPrepMethodId(),
                        ngsRelatedData.isXenograft(),
                        ngsRelatedData.getLibraryPrep().name(),
                        ngsRelatedData.getNtExtractionMethod().getId()
                    ) :
                    null;
    ...
    public FileItem readFileDetailsWithConditions(long actor, final long fileId, long experimentId) {
        ...
        checkAccess(
            ruleValidator.userHasReadPermissionsOnFilePredicate(actor).apply(Util.FILE_FROM_ID.apply(fileId)),
            "User has no permissions to read file"
        );
        final RawFile rawFile = checkPresence(
            filesRepository.findByFileAndExperiment(fileId, experimentId),
    ...
    public List<ShortFileWithConditions> readFilesAndConditionsForExperiment(long actor, long experimentId) {
        ...
        for (RawFile rawFile : filesData) {
            ...
            final ImmutableList<ConditionItem> conditions =
                from(fileConditions).filter(new Predicate<SampleCondition>() {
                    @Override
            ...
            final ShortFileWithConditions shortFileWithConditions =
                new ShortFileWithConditions(fileMetaData.getId(), fileMetaData.getName(), joinedConditions);
    ...
    protected Function<ExperimentFileTemplate, ? extends ShortExperimentFileItem> shortInfoFileTransformer(
        ...
        return new Function<ExperimentFileTemplate, ExtendedShortExperimentFileItem>() {
            ...
            public ExtendedShortExperimentFileItem apply(ExperimentFileTemplate input) {
                ...
                final Function<SampleCondition, ConditionItem> conditionItemTransformer =
                    sampleConditionTransformer(experiment);
                for (PrepToExperimentSample prepToExperimentSample : ((RawFile) input).getPreparedSample()
                    .getSamples()) {
                    final ExperimentSample sample = prepToExperimentSample.getExperimentSample();
                    final SampleCondition sampleCondition =
                        sample.getSampleConditions().isEmpty() ? SampleCondition.createUndefinedCondition(
                            experiment, ImmutableList.of(sample)) : sample.getSampleConditions().iterator().next();
                    samples.add(new ExperimentShortSampleItem(
                        sample.getId(),
                        sample.getName(),
                        prepToExperimentSample.getType().name(),
                        conditionItemTransformer.apply(sampleCondition),
                        sample.getAnnotationValues().stream()
                            .map(v -> new AnnotationItem(
                                v.getId(),
                                v.getName(),
                                v.getValue(),
                                v.getUnits(),
                                v.getType().equals(INTEGER)
                            ))
                            .collect(Collectors.toCollection(() -> new ArrayList<>()))
                    ));
                }
                final FileMetaDataTemplate fileMetaData = input.getFileMetaData();
                return new ExtendedShortExperimentFileItem(
                    fileMetaData.getId(),
            ...
            private Function<SampleCondition, ConditionItem> sampleConditionTransformer(
                final ActiveExperiment experiment
            ) {
                return input -> new ConditionItem(input.getId(), input.getName(), experiment.getName());
            }
        };
    }
    ...
    public List<ShortExperimentFileItem> readFilesInOtherExperiments(long actor, long experiment) {
        return from(experimentRepository.filesInOtherExperiments(experiment))
            .transform(input -> new ShortExperimentFileItem(
                input.getId(),
                input.getName(),
                ImmutableList.<ConditionItem>of(),
                ImmutableList.<AnnotationItem>of()
            ))
            .toList();
    }
    ...
    public AttachmentsReaderTemplate.AttachmentItem readExperimentAnnotationAttachment(long actor, long experimentID) {
        ...
        return new AttachmentsReaderTemplate.AttachmentItem(
            annotationAttachment.getId(),
            annotationAttachment.getName(),
            annotationAttachment.getSizeInBytes(),
            annotationAttachment.getUploadDate(),
            annotationAttachment.getOwner().getId()
        );
    }

    @Override
    public AttachmentsReaderTemplate.AttachmentItem readAnnotationAttachmentDetails(long actor,
                                                                                    long annotationAttachmentId) {
        final AnnotationAttachment annotationAttachment =
            checkPresence(annotationAttachmentRepository.findOne(annotationAttachmentId));
        return new AttachmentsReaderTemplate.AttachmentItem(
            annotationAttachment.getId(),
            annotationAttachment.getName(),
            annotationAttachment.getSizeInBytes(),
    ...
    @Override
    protected InstrumentItem afterReadInstrument(
        long actor,
        SingleResultBuilder<AccessedInstrument<Instrument>, InstrumentItem> instrumentItemBuilder
    ) {
    ...
    public FileItem transformFile(ActiveFileMetaData activeFileMetaData) {
        ...
        return new FileItem(defaultTransformed, activeFileMetaData.getArchiveId(), transformStorageStatus(
            activeFileMetaData.getStorageData().getStorageStatus(),
    ...
    @Override
    public InstrumentItem transformInstrument(AccessedInstrument<Instrument> accessedInstrument) {

        final Instrument instrument = accessedInstrument.instrument;
        final InstrumentItemTemplate instrumentItemTemplate =
            instrumentHelper.getDefaultTransformer().apply(accessedInstrument);
        final ImmutableList<LockMzItem> lockMasses =
            from(transform(instrument.getLockMasses(), LOCK_MZ_ITEM_FUNCTION)).toList();
    ...
    public ExperimentShortInfoDetailed readExperimentShortInfo(long actor, long experimentId) {
        ...
        final ExperimentCategory experimentCategory =
            experimentRepository.findOne(experimentId).getExperimentCategory();
