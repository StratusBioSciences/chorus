...
public class AbstractTest extends AbstractTestNGSpringContextTests {
    ...
    public static final String ACCESSION_PARSE_RULE = ">\\([^ ,]*\\)";
    public static final String DESCRIPTION_PARSE_RULE = ">[^ ]* \\(.*\\)";
    public static final Set<String> NGS_EXP_DOWNLOAD_DATA_FIELD_NAMES =
        Sets.newHashSet("Xenograft", "Multiplexing", "Library Prep");
    ...
    private long accessionParseRule;
    ...
    public long createInstrumentAndExperiment(
        long user,
        long lab,
        long project,
        List<ExperimentManagementTemplate.MetaFactorTemplate> noFactors,
        List<com.infoclinika.mssharing.model.write.FileItem> fileItems
    ) {
        ...
        final ExperimentInfo.Builder builder = experimentInfo().project(project)
            .lab(lab)
            .billLab(lab)
            .is2dLc(false)
            .restriction(restriction(user, fileItems.get(0).id))
            .factors(noFactors)
            .files(fileItems)
    ...
    public long createExperimentWithInstrumentRestriction(long user, long project, long instrument) {
        final long file = uc.saveFile(user, instrument);
        final Instrument instrumentEntity = repos.instrumentRepository.findOne(instrument);
        final ExperimentInfo.Builder builder = experimentInfo()
            .project(project)
            .lab(instrumentEntity.getLab().getId())
            .billLab(instrumentEntity.getLab().getId())
            .is2dLc(false)
            .restriction(restrictionFromModelAndInstrument(instrumentEntity.getModel().getId(), instrument))
    ...
    public long createInstrumentAndExperimentWithOneFile(long user, long lab, long project) {
        ...
        if (lab == uc.getLab3()) {
            ...
            final Optional<Long> labInstrument = createInstrumentAndApproveIfNeeded(
                user,
                lab,
                anyInstrumentModelByVendor(anyVendor()),
                instrumentDetails()
    ...
    public long createExperimentWithLabels(long user,
                                           long project,
                                           long experimentType,
                                           ExperimentLabelsInfo experimentLabels) {
    ...
    public long createExperimentWithName(long user, long project, String name) {
        ...
        final ExperimentInfo.Builder builder = experimentInfo(name).project(project)
            .lab(lab)
            .billLab(lab)
            .is2dLc(false)
            .experimentLabels(new ExperimentLabelsInfo())
    ...
    public ExperimentInfo.Builder experimentInfo(String name, long species) {
        return new ExperimentInfo.Builder().name(name)
    ...
    protected long abSciexInstrumentModel() {
        ...
        if (model == null) {
            ...
            return predefinedDataCreator.instrumentModel(
                AB_SCIEX,
                instrumentType11,
                msStudyType,
    ...
    public void setFeature(ApplicationFeature feature, boolean enabled) {
        featuresHelper.set(feature, enabled);
    }

    @Transactional
    public void setFeaturePerLab(ApplicationFeature feature, Collection<Long> labIds) {
        featuresHelper.setForLabs(feature, Sets.newHashSet(labIds));
    }
    ...
    public void updateExperimentFiles(long user, long experiment, long file) {
        ...
        builder.addAll(transform(
            ...
            new Function<FileItemTemplate, com.infoclinika.mssharing.model.write.FileItem>() {
                @Override
    ...
    public void addFilesToExperiment(long user,
                                     long experiment,
                                     final List<ExperimentManagementTemplate.MetaFactorTemplate> factors,
                                     ImmutableList<com.infoclinika.mssharing.model.write.FileItem> fileItems,
                                     final List<String> factorValuesOfNewFactorsForOldSamples) {
        final ImmutableList.Builder<com.infoclinika.mssharing.model.write.FileItem> builder = ImmutableList.builder();
        final ExperimentItem experimentItem = detailsReader.readExperiment(user, experiment);
        final Iterable<com.infoclinika.mssharing.model.write.FileItem> oldFiles = transform(
            experimentItem.files,
            new Function<FileItemTemplate, com.infoclinika.mssharing.model.write.FileItem>() {
                @Override
                public com.infoclinika.mssharing.model.write.FileItem apply(FileItemTemplate input) {
                    ...
                    final ExperimentPreparedSampleItem preparedSample =
                        new ExperimentPreparedSampleItem(fileItem.preparedSample.name, updatedSamples);
                    return new com.infoclinika.mssharing.model.write.FileItem(
                        input.id,
                        false,
        ...
        final ExperimentInfo.Builder infoBuilder = new ExperimentInfo.Builder()
            ...
            .files(builder.build())
            .experimentLabels(new ExperimentLabelsInfo(
                experimentItem.labels.lightLabels,
                experimentItem.labels.mediumLabels,
                experimentItem.labels.heavyLabels
            ))
    ...
    protected long createExperimentForALISRun(long bob) {
        long project = uc.createProject(bob);
        long instrument = createInstrumentAndApproveIfNeeded(bob, uc.getLab3());
        long sampleId = uc.saveFileWithName(bob, instrument, "sample.raw");
        long refconId = uc.saveFileWithName(bob, instrument, "refcon.raw");

        long ex = createExperiment(bob, project, uc.getLab3(), noFactoredFiles(Lists.newArrayList(sampleId, refconId)));
        return ex;
    }
    ...
    protected long brukerInstrumentModel() {
        ...
        if (model == null) {
            ...
            return predefinedDataCreator.instrumentModel(BRUKER, instrumentType11, msStudyType, BRUKER_INSTRUMENT_MODEL,
                true, false, BRUKER_EXTENSIONS
            );
        }
        return model;
    }

    protected long ngsInstrumentModel() {
        Long model = getInstrumentModelByNames(Data.NGS_VENDOR, Data.NGS_INSTRUMENT);
        if (model == null) {
            //initialize NGS instrument model
            return predefinedDataCreator.instrumentModel(
                Data.NGS_VENDOR,
                Data.NGS_TYPE,
                Data.NGS_STUDY_TYPE,
    ...
    @Transactional
    public int ngsExperimentType() {
        if (repos.ngsExperimentTypeRepository.count() > 0) {
            return repos.ngsExperimentTypeRepository.findAll().get(0).getId();
        }
        final NgsExperimentType experimentType =
            repos.ngsExperimentTypeRepository.save(new NgsExperimentType(Data.NGS_EXP_TYPE_NAME, new ArrayList<>()));
        return experimentType.getId();
    }

    @Transactional
    public long ntExtractionMethod() {
        if (repos.ntExtractionMethodRepository.count() > 0) {
            return repos.ntExtractionMethodRepository.findAll().get(0).getId();
        }
        final NtExtractionMethod ntExtractionMethod = new NtExtractionMethod();
        ntExtractionMethod.setTitle(NGS_NT_EXTRACTION_METHOD_NAME);
        return repos.ntExtractionMethodRepository.save(ntExtractionMethod).getId();
    }

    @Transactional
    public int ngsExperimentTypeWithPrepMethods() {
        final ExperimentPrepMethod firstPrepMethod = repos.experimentPrepMethodRepository.save(new ExperimentPrepMethod(
            "first-prep-method-" + System.currentTimeMillis()));
        final ExperimentPrepMethod secondPrepMethod =
            repos.experimentPrepMethodRepository.save(new ExperimentPrepMethod(
                "second-prep-method-" + System.currentTimeMillis()));
        final NgsExperimentType experimentType = repos.ngsExperimentTypeRepository.save(
            new NgsExperimentType(Data.NGS_EXP_TYPE_NAME, Lists.newArrayList(firstPrepMethod, secondPrepMethod))
        );
        return experimentType.getId();
    }
    ...
    public void tearDown() {
        ...
        for (int i = 0; i < repositories.size(); i++) {
            ...
            try {
                ...
                log.error("Failed on deletion. Repository - '{}'", repo.getClass().getName());
    ...
    public void setUp() {
        ...
        final String description = "Translation Step";

        secondAdminId = predefinedDataCreator.admin("Mark2", "Thomson2", Data.ADMIN_EMAIL_2, "1234");
        final long chargeableItem = billingManagement.createChargeableItem(
            ARCHIVE_PRICE,
            BillingFeature.ARCHIVE_STORAGE,
            1,
            BillingChargeType.PER_GB
        );//cents
        final long chargeableItem1 = billingManagement.createChargeableItem(
            ANALYSE_PRICE,
            BillingFeature.ANALYSE_STORAGE,
            1,
            BillingChargeType.PER_GB
        );
        final long chargeableItem3 = billingManagement.createChargeableItem(
            DOWNLOAD_PRICE,
            BillingFeature.DOWNLOAD,
            1,
            BillingChargeType.PER_GB
        );//cents
        final long chargeableItem5 = billingManagement.createChargeableItem(
            DOWNLOAD_PRICE,
            BillingFeature.PUBLIC_DOWNLOAD,
            1,
            BillingChargeType.PER_GB
        );//cents
        final long storageVolumes = billingManagement.createChargeableItem(
            STORAGE_VOLUMES_FEATURE_PRICE,
            BillingFeature.STORAGE_VOLUMES,
            1,
            BillingChargeType.PER_GB
        );//cents
        final long archiveStorageVolumes = billingManagement.createChargeableItem(
            ARCHIVE_STORAGE_VOLUMES_FEATURE_PRICE,
            BillingFeature.ARCHIVE_STORAGE_VOLUMES,
            1,
            BillingChargeType.PER_GB
        );//cents

        final HashSet<FileExtensionItem> extensions = newHashSet(new FileExtensionItem(
            ".raw",
            "",
            Collections.<String, AdditionalExtensionImportance>emptyMap()
        ));
        predefinedDataCreator.instrumentModel(
            vendor1,
            instrumentType11,
            msStudyType,
            instrumentModel111,
        ...
        final HashSet<FileExtensionItem> extensions3 = newHashSet(new FileExtensionItem(
            ".raw",
            ".raw",
            Collections.<String, AdditionalExtensionImportance>emptyMap()
        ));
        ...
        final Set<FileExtensionItem> microArraysExtensions = newHashSet(new FileExtensionItem(
            ".CEL.gz",
            "",
            Collections.<String, AdditionalExtensionImportance>emptyMap()
        ));
        ...
        predefinedDataCreator.experimentType("ALIS Screening", false, false);
        ...
        uc = new UseCase(
            writeServices,
            adminId,
            instrumentCreationHelper,
            experimentCreationHelper,
            dashboardReader,
            repos
        );
        setBilling(false);
        ...
        experimentLabelWithRAminoAcid =
            experimentLabelManagement.createLabel(new ExperimentLabelManagement.ExperimentLabelInfo(
                "R",
                "Arg10",
                experimentLabelType
            ));
    ...
    protected void initFailedEmailsNotifiers() {
        ...
        repos.failedEmailsNotifierRepository.save(of(
            new FailedMailNotificationReceiver("example@email.com"),
            new FailedMailNotificationReceiver("example2@email.com")
        ));
    }
    ...
    public long getAccessionParseRule() {
        return accessionParseRule;
    }

    public long getDescriptionParseRule() {
        return descriptionParseRule;
    }
    ...
    protected long getFileImageProcessingStepType() {
        return fileImageProcessingStepType;
    }
    ...
    protected com.infoclinika.mssharing.model.write.FileItem fileItem(long id) {
        return new com.infoclinika.mssharing.model.write.FileItem(id, false, 0, preparedSample(id));
    }

    protected long createFile(long user, long instrument, String name) {
        final long file = instrumentManagement.createFile(
            user,
            instrument,
            new FileMetaDataInfo(name, 0, "", null, anySpecies(), false)
        );
        uc.updateFileContent(user, file);
        return file;
    }

    protected static ImmutableMap<String, ExperimentSampleItem> extractSampleNameToSample(
