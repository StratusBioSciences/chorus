...
abstract class AbstractSharingTest extends AbstractTest {
    ...
    public void checkHasAccessToSharedFile(long user, long lab, final long file) {
        if (!Iterables.any(fileReader.readFiles(user, Filter.SHARED_WITH_ME), new Predicate<FileLine>() {
            @Override
    ...
    public long projectByUser(long user, long lab) {
        return studyManagement.createProject(user, new ProjectInfo(generateString(), "DNA", "Some proj", lab));
    }
