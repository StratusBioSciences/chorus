...
public class ExperimentTest extends AbstractStudyTest {
    ...
    public void testCreateExperimentForUserWithoutLab() {
        ...
        addFilesToExperiment(bob, experiment,
            ...
            of(new com.infoclinika.mssharing.model.write.FileItem(
                file2,
                false,
                0,
                preparedSample(file2, ImmutableSet.of(sampleWithFactors))
            )), of("3")
        );
    ...
    public void testFilesAreFilteredByInstrumentAndSpecie() {
        ...
        assertEquals(
            size(experimentCreationHelper.availableFilesByInstrument(bob, anotherSpecie(specie), instrument)),
            1
        );
        assertEquals(
            size(experimentCreationHelper.availableFilesByInstrument(bob, unspecifiedSpecie(), instrument)),
            2
        );
    }
    ...
    public void testExperimentDataDetails() {
        ...
        final long projectId = studyManagement.createProject(
            bob,
            new ProjectInfo(projectName, generateString(), generateString(), uc.getLab3())
        );
        ...
        final ExperimentInfo.Builder builder =
            new ExperimentInfo.Builder().name(experimentName).description(experimentDescription)
    ...
    public void testCreatingExperimentWithFactoredFile() {
        ...
        addFilesToExperiment(bob, experiment,
            ...
            of(new com.infoclinika.mssharing.model.write.FileItem(
                ...
                false,
                0,
                preparedSample(file, ImmutableSet.of(sampleItem))
            )), of(factorValue)
        );
    ...
    public void testCreatingExperimentWithAnnotatedFile() {
        ...
        addFilesToExperiment(bob, experiment,
            ...
            of(new com.infoclinika.mssharing.model.write.FileItem(
                ...
                false,
                0,
                preparedSample(file, ImmutableSet.of(sample))
            )), Collections.<String>emptyList()
        );

        final ExperimentItem experimentItem = detailsReader.readExperiment(bob, experiment);

        final FluentIterable<DetailsReaderTemplate.FileItemTemplate> annotatedFiles =
            from(experimentItem.files).filter(new Predicate<DetailsReaderTemplate.FileItemTemplate>() {
                @Override
    ...
    public void testAnnotationsMissed() {
        ...
        final long project = uc.createProject(bob, uc.getLab3());
        final ExperimentSampleItem sample = sampleWithNoFactors(file);
        final ExperimentInfo.Builder builder = new ExperimentInfo.Builder().name("")
            .description("")
            ...
            .restriction(restriction(bob))
            .factors(NO_FACTORS)
            .files(of(new com.infoclinika.mssharing.model.write.FileItem(
                file,
                false,
                0,
                preparedSample(file, ImmutableSet.of(sample))
    ...
    public void testFactoredValuesSetProperly() {
        ...
        addFilesToExperiment(bob, experiment,
            ...
            of(
                new com.infoclinika.mssharing.model.write.FileItem(
                    file1,
                    false,
                    0,
                    preparedSample(file1, ImmutableSet.of(sample1))
                ),
                new com.infoclinika.mssharing.model.write.FileItem(
                    file2,
                    false,
                    0,
                    preparedSample(file2, ImmutableSet.of(sample2))
                )
            ),
            of("33", "4", "8")
        );
        ...
        final ImmutableMap<String, ExperimentSampleItem> sampleNameToSampleMap =
            extractSampleNameToSample(experimentItem);
        ...
        assertTrue(deepEquals(experimentItem.factorValues, new String[][] {
            {"1.23", "a bcd", "-1"},
            {"23", "4", "7"},
            {"33", "4", "8"}
        }));
    ...
    public void testUserCantCreateExperimentAlreadyExistedName() {
        ...
        final ExperimentInfo.Builder duplicatedBuilder = new ExperimentInfo.Builder().name("Duplicated title")
    ...
    public void testAddingFilesToExperiment() {
        ...
        sort(experimentItem.factorValues, comparingInt(o -> parseInt(o[0])));

        assertTrue(deepEquals(experimentItem.factorValues, new String[][] {{"1"}, {"2"}, {"3"}}));
        final ImmutableMap<String, ExperimentSampleItem> sampleNameToSampleMap =
            extractSampleNameToSample(experimentItem);
    ...
    public void testRemoveFileFromExperiment() {
        final long bob = uc.createLab3AndBob();
        final long project =
            studyManagement.createProject(bob, new ProjectInfo(PROJECT_TITLE, "DNA", "Some proj", uc.getLab3()));
        final long experiment = createInstrumentAndExperimentWithOneFile(bob, uc.getLab3(), project);
    ...
    public void testRemovingFactorFromExperiment() {
        ...
        addFilesToExperiment(bob, experiment, of(factor(experiment)), of(
            new com.infoclinika.mssharing.model.write.FileItem(
                ...
                false,
                0,
                preparedSample(file3, ImmutableSet.of(sample3))
            ),
            new com.infoclinika.mssharing.model.write.FileItem(
                file4,
                false,
                0,
                preparedSample(file4, ImmutableSet.of(sample4))
            )
        ), of("50"));
        final ExperimentItem experimentItem = detailsReader.readExperiment(bob, experiment);
        assertTrue(deepEquals(experimentItem.factorValues, new String[][] {{"50"}, {"50"}, {"50"}, {"42"}, {"98"}}));
    ...
    public void testAddingFactorToExperiment() {
        ...
        final ExperimentItem experimentItem = detailsReader.readExperiment(bob, experiment);
        assertTrue(deepEquals(experimentItem.factorValues, new String[][] {{"3", "50"},
            {"3", "50"}, {"3", "50"},
            {"1", "42"}, {"2", "98"}
        }));
    ...
    public void testShouldNotAllowToCreateFactorWithoutName() {
        ...
        addFilesToExperiment(
            bob,
            experiment,
            of(new ExperimentManagementTemplate.MetaFactorTemplate("", "", false, experiment)),
            of(
                new com.infoclinika.mssharing.model.write.FileItem(
                    file1,
                    false,
    ...
    public long experiment(long user, long lab) {
        final long project =
            studyManagement.createProject(user, new ProjectInfo(PROJECT_TITLE, "DNA", "Some proj", lab));
    ...
    public void testNotAllowCreateExperimentWithoutFiles() {
        ...
        final ExperimentInfo.Builder builder = new ExperimentInfo.Builder().name("Duplicated title")
            ...
            .restriction(restriction(bob))
            .factors(NO_FACTORS)
            .files(Collections.<com.infoclinika.mssharing.model.write.FileItem>emptyList())
            .bounds(new AnalysisBounds())
            .lockMasses(NO_LOCK_MASSES);
    ...
    public void testCantUpdateExperimentWithInvalidFilesAndSpeciesCombination() {
        ...
        final ExperimentInfo.Builder builder = new ExperimentInfo.Builder().name(generateString())
            ...
            .experimentType(anyExperimentType())
            .specie(anotherSpecies)
            .billLab(uc.createLab3())
            .project(details.project)
            .lab(details.lab)
            .billLab(uc.createLab3())
            .is2dLc(details.is2dLc)
            .restriction(restrictionForExperiment(details))
            .factors(NO_FACTORS)
            .files(noFactoredFile(file))
            .lockMasses(NO_LOCK_MASSES)
            .experimentLabels(new ExperimentLabelsInfo());
    ...
    public void testCantUpdateExperimentWithInvalidFilesAndInstrumentModelCombination() {
        ...
        final ExperimentInfo.Builder builder = new ExperimentInfo.Builder().name(generateString())
            ...
            .billLab(uc.createLab3())
            .is2dLc(details.is2dLc)
            .restriction(restrictionFromModelAndInstrument(anotherModel, details.instrument.get()))
            .experimentLabels(new ExperimentLabelsInfo())
            .factors(NO_FACTORS)
            .files(noFactoredFile(file))
            .lockMasses(NO_LOCK_MASSES);
    ...
    public void createExperimentWithLabelsWithRightExperimentType() {
        ...
        final List<ExperimentLabelReader.ExperimentLabelItem> allLabels =
            experimentLabelReader.readLabels(experimentLabelType);
        ...
        final long experiment = createExperimentWithLabels(
            bob,
            uc.createProject(bob),
            experimentTypeLabeled(),
            new ExperimentLabelsInfo(lightLabels, mediumLabels, heavyLabels)
        );
        final ExperimentItem experimentItem = detailsReader.readExperiment(bob, experiment);
    ...
    public void createExperimentWithLabelsWithExperimentTypeWhichDoesntSupportLabels() {
        ...
        final long experiment = createExperimentWithLabels(
            bob,
            uc.createProject(bob),
            experimentTypeNotLabeled(),
            new ExperimentLabelsInfo(lightLabels, mediumLabels, newArrayList())
        );
    }


    private static final Function<ExperimentLabelReader.ExperimentLabelItem, Long> EXTRACT_ID =
        new Function<ExperimentLabelReader.ExperimentLabelItem, Long>() {
            @Override
    ...
    private long createFile(long bob, long species, long model, String name) {
        final long file = instrumentManagement.createFile(
            bob,
            createInstrumentByModel(bob, uc.getLab3(), model),
            new FileMetaDataInfo(name, 0, "", null, species, false)
        );
    ...
    private long createExperiment(long bob, long project, long instrumentModel, long file, long species) {
        final ExperimentInfo.Builder builder = experimentInfo(species).project(project)
            .lab(uc.getLab3())
            .billLab(uc.getLab3())
            .is2dLc(false)
    ...
    @Test
    public void testCreateExperimentWithNgsData() {

        final long actor = uc.createLab3AndBob();
        final long experimentId = createNgsExperiment(actor);
        final ExperimentItem experimentItem = detailsReader.readExperiment(actor, experimentId);

        Assert.assertNotNull(experimentItem);
        Assert.assertNotNull(experimentItem.ngsRelatedInfo);
    }

    @Test
    public void testReadExperimentDownloadDataWithNgs() {

        final long actor = uc.createLab3AndBob();
        final long experimentId = createNgsExperiment(actor);
        final ChorusExperimentDownloadData downloadData =
            downloadHelper.readExperimentDownloadData(actor, experimentId);

        Assert.assertNotNull(downloadData);
        final Collection<String> fieldNames = downloadData.getFieldNames();
        Assert.assertTrue(fieldNames.containsAll(NGS_EXP_DOWNLOAD_DATA_FIELD_NAMES));
    }

    private long createNgsExperiment(long actor) {

        final long project = createPrivateProject(actor, uc.getLab3());
        final long anySpecies = anySpecies();
        final long model = ngsInstrumentModel();
        final long file = createFile(actor, anySpecies, model);
        final long experimentType = anyExperimentType();

        final ExperimentInfo experimentInfo = experimentInfo(anySpecies)
            .project(project)
            .lab(uc.getLab3())
            .billLab(uc.getLab3())
            .is2dLc(false)
            ...
            .lockMasses(NO_LOCK_MASSES)
            .experimentLabels(new ExperimentLabelsInfo())
            .ngsRelatedInfo(new NgsRelatedExperimentInfo(
                ngsExperimentType(),
                false,
                1,
                1,
