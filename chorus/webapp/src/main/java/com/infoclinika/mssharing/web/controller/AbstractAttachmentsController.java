...
public abstract class AbstractAttachmentsController extends AbstractFileUploadController {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(AbstractAttachmentsController.class);
    ...
    protected AbstractAttachmentsController(
        AttachmentsReaderTemplate attachmentsReader,
    ...
    protected void postAttachmentToResponse(
        AttachmentItem attachmentItem,
        HttpServletRequest request,
        HttpServletResponse response,
        Function<Long, AttachmentItem> attachmentItemFunction
    ) throws IOException {

        //todo[tymchenko]: code smell; Refactor File storage API to avoid this
        final StoredFile file =
            abstractStorageHelper.getContent(attachmentItem.id, attachmentItem.ownerId).get().storedFile;
        final InputStream is = file.getInputStream();

        response.setContentType("application/octet-stream");
        response.setHeader(
            "Content-Disposition",
            encodeContentDisposition(attachmentItem.name, request.getHeader(USER_AGENT))
        );
