...
(function () {
    ...
    angular.module(
        "general-requests-front",
        ["general-requests", "security-front", "requests-details", "front-end", "instruments-front", "error-catcher"]
    )
        .controller("generalRequests", function ($scope,
                                                 $route,
                                                 $rootScope,
                                                 $location,
                                                 GeneralRequests,
                                                 requestsUpdatedNotificationService,
                                                 refuseConfirmationDialog,
                                                 Inbox,
                                                 Outbox,
                                                 GeneralRequestsControllerCommon) {
            ...
            $scope.refreshInbox = function () {
                $scope.inbox = Inbox.query(function (items) {
                    ...
                    if ($location.$$url.match(/\/inbox.*/)) {
                        ...
                        var inboxSetup = GeneralRequestsControllerCommon(
                            $scope,
                            idsToMarkAsRead,
                            Inbox,
                            "#inbox-mark-as-read-confirmation"
                        );
            ...
            $scope.refreshOutbox = function () {
                $scope.outbox = Outbox.query(function (items) {
                    ...
                    if ($location.$$url.match(/\/outbox.*/)) {
                        ...
                        var outboxSetup = GeneralRequestsControllerCommon(
                            $scope,
                            idsToMarkAsRead,
                            Outbox,
                            "#outbox-mark-as-read-confirmation"
                        );
            ...
            $scope.showDetails = function (request) {
                var ids = request.id.split(/(\d+)/);
                let isInstrumentStrategy = ids[0] === "InstrumentStrategy";
                let isLabCreationStrategy = ids[0] === "LabCreationStrategy";
                let isInstrumentCreationStrategy = ids[0] === "InstrumentCreationStrategy";
                var path = "";
                if (isInstrumentStrategy) {
                    path = "instrument/" + parseInt(ids[1]) + "/" + parseInt(ids[3]);
                } else if (isLabCreationStrategy) {
                    path = "lab/" + parseInt(ids[1]);
                } else if (isInstrumentCreationStrategy) {
                    path = "instrument-creation/" + parseInt(ids[1]);
                }
            ...
            $scope.showDetailsButton = function (request) {
                return request.availableActions === "APPROVE_REFUSE" &&
                    !request.id.match(/(.*LabMembership.*)|(.*ProjectSharing.*)|(.*CopyProject.*)/);
            };
        })
        ...
        .controller("copy-project-confirmation", function ($scope,
                                                           $rootScope,
                                                           $location,
                                                           Inbox,
                                                           $routeParams,
        ...
        .controller("lab-inbox-details", function ($scope,
                                                   $rootScope,
                                                   $location,
                                                   RequestsDetails,
                                                   $routeParams,
            ...
            $scope.refuse = function () {
                refuseConfirmationDialog(
                    RequestsDetails.refuseLabCreation,
                    {requestId: $routeParams.id, comment: ""},
                    function () {
                        requestsUpdatedNotificationService.broadcastItem();
                    }
                );
        ...
        .controller("instrument-creation-details", function ($scope,
                                                             $rootScope,
                                                             RequestsDetails,
                                                             $routeParams,
                                                             requestsUpdatedNotificationService,
            ...
            $scope.refuse = function () {
                refuseConfirmationDialog(
                    RequestsDetails.refuseInstrumentCreation,
                    {requestId: $routeParams.id, comment: ""},
                    function () {
                        requestsUpdatedNotificationService.broadcastItem();
                    }
            ...
            $scope.isSerialNumberDuplicated = function () {
                if (!$scope.instrument.details.serialNumber) {
                    return false;
                }
                var instruments = $.grep(instrumentsByLab, function (item) {
        ...
        .controller("instrument-inbox-details", function ($scope,
                                                          $rootScope,
                                                          $location,
                                                          RequestsDetails,
                                                          $routeParams,
            ...
            RequestsDetails.get(
                {path: "instrument", request: $routeParams.instrument, requester: $routeParams.user},
                function (instrument) {
            ...
            $scope.approve = function () {
                RequestsDetails.approveInstrument(
                    {request: $routeParams.instrument, requester: $routeParams.user},
                    function () {
                        setTimeout(function () {
                            $(".modal").modal("hide");
                        }, 0);
            ...
            $scope.refuse = function () {
                refuseConfirmationDialog(
                    RequestsDetails.refuseInstrument,
                    {instrumentId: $routeParams.instrument, requesterId: $routeParams.user, comment: ""},
                    function () {
                        requestsUpdatedNotificationService.broadcastItem();
                    }
                );
