...
angular
    .module(
        "upload-front",
        ["upload-back", "instruments-back", "experiments-back", "files-back", "formatters", "background-upload", "laboratories",
            "front-end", "experiments-back", "error-catcher", "enums", "feature-service"]
    )
    .controller("new-upload", function ($scope,
                                        $rootScope,
                                        OperatedInstruments,
                                        Upload,
                                        ExperimentSpecies,
            ...
            function init() {
                ...
                function setupWatchers() {
                    ...
                    function onUploadItemsChanged() {
                        ...
                        if (!$rootScope.isZipping && $scope.step == 4 && $.grep(
                    ...
                    function onSelectedTechnologyTypeIdChanged(typeId, previousTypeId) {
                        ...
                        if (!typeId || typeId == previousTypeId) {
                            return;
                        }
                    ...
                    function onSelectedVendorIdChanged(vendorId, previousVendorId) {
                        ...
                        if (!vendorId || vendorId == previousVendorId || !$scope.selectedTechType.id) {
                            return;
                        }
                    ...
                    function updateAvailableInstruments() {
                        ...
                        $scope.selectedInstrument.id =
                            $scope.filteredInstruments.length > 0 && getDefaultOptionValue($scope.filteredInstruments) ||
                            DEFAULT_INSTRUMENT.id;
                    }
                    ...
                    function updateInstrumentModels() {
                        ...
                        InstrumentModels.getByTechnologyTypeAndVendor({
                            vendor: $scope.selectedVendor.id,
                            technologyType: $scope.selectedTechType.id
                ...
                function filterDuplicatedFiles(files) {
                    var filesMap = {};
                ...
                function initDragAndDropHelper() {
                    dragNDropHelper = new DragNDropHelper({
                        ...
                        allowedExtensionsFn: function () {
                            return isFolderArchiveSupport() && !isMultipleFilesSupport() ?
                                [] :
                        ...
                        onFilesDroppedCallback: function () {
                            ...
                            $rootScope.filesToUpload = filterDuplicatedFiles($rootScope.filesToUpload);
                ...
                function initLabelsTableModel() {
                    labelsTable = new TableModel(
                        ...
                        function (x, y) {
                            var isMulti = isMultipleFilesSupport();
                            return isMulti ?
                                $rootScope.multipleFilesItems[y].original.labels :
                                $rootScope.filesToUpload[y].labels;
                        },
                        function (x, y, value) {
                            var isMulti = isMultipleFilesSupport();
                            if (isMulti) {
                ...
                function initFromFirstStep() {
                    ...
                    ExperimentSpecies.query(function (items) {
                        $scope.species = items;
                        var unspecified = items.find(function (specie) {
                            return specie.unspecified;
                        });

                        $scope.defaultSpecie.id = unspecified.id;
                    });
            ...
            function showInstrumentSelector() {
                return $scope.filteredInstruments.length > 1 ||
                    $scope.filteredInstruments.length == 1 &&
                    $scope.filteredInstruments[0].name != DEFAULT_INSTRUMENT_NAME;
            }
            ...
            function zipAndUploadMultiple() {
                $rootScope.zipAndUploadHelper.processZipping(
                    $rootScope.multipleFilesItems,
                    function (itemsForZipAndUpload, currentIndex) {
                        ...
                        if (isFolderArchiveSupport()) {
                            $rootScope.zipHelper.zipDirWithFiles(
                                item.original.originalFile,
                                item.uploadName,
                                allFilesToZipEntry,
                                completeZip,
                                zipFailed
            ...
            function zipAndUpload() {
                ...
                $rootScope.zipAndUploadHelper.processZipping(
                    $rootScope.filesToUpload,
                    function (itemsForZipAndUpload, index) {
    ...
    .controller("resume-upload", function ($scope, $rootScope, uploadStats,
                                           updateIndicators, UploadItem, UnfinishedUploads, startNewUpload,
            ...
            $scope.setFile = function (element) {
                $scope.$apply(function () {
                    ...
                    if (item.name != element.files[0].name) {
                        ...
                        $scope.errorMessage =
                            "Size of file " + item.name + " should be " + $filter("fileSize")(item.size);
                    } else {
