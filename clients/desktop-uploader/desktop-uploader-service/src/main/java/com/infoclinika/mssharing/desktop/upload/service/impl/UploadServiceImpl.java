package com.infoclinika.mssharing.desktop.upload.service.impl;
...
import com.infoclinika.mssharing.desktop.upload.service.api.ZipService;
import com.infoclinika.mssharing.desktop.upload.service.api.listener.UploadListener;
...
public class UploadServiceImpl implements UploadService {
    ...
    @Override
    public void upload(List<UploadFileItem> uploadItems, UploadConfig config, UploadZipListener listener) {
        LOGGER.info("Start files upload: {}", uploadItems);

        final PauseSemaphore pauseSemaphore = config.getPauseSemaphore();

        final ExecutorService executorService = Executors.newSingleThreadExecutor();
        executorService.submit(() -> {
            boolean error = false;
            for (UploadFileItem item : uploadItems) {
                try {
                    ...
                    error = true;
                    listener.onUploadLimitExceeded();
                } catch (Exception e) {
                    error = true;
                    LOGGER.error("Error while uploading {}", item, e);
                }
            }

            if (!error) {
                listener.onAllItemsUploaded(uploadItems);
            }
        });
        executorService.shutdown();
    }

    private void upload(UploadFileItem item, UploadConfig config, UploadZipListener listener) {
    ...
    @Override
    public void pause(UploadConfig uploadConfig) {
        final PauseSemaphore pauseSemaphore = uploadConfig.getPauseSemaphore();
        pauseSemaphore.setPause(true);
    }

    @Override
    public void resume(UploadConfig uploadConfig) {
        final PauseSemaphore pauseSemaphore = uploadConfig.getPauseSemaphore();
        pauseSemaphore.setPause(false);
    }
    ...
    private boolean postUploadRequest(UploadFileItem item, long instrumentId, boolean isArchive,
                                      UploadListener listener) {
        LOGGER.info("Post upload file request. File: {} Size: {}", item.getName(), item.getFileSize());
    ...
    private void postUploadCompleteRequest(UploadFileItem item,
        ...
        try {
            ...
            LOGGER.info("Confirmation response for item: {} -> confirmed - {}", item.getName(), response.isConfirmed());
