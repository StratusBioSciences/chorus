...
public abstract class DefaultAccessedInstrumentReader<INSTRUMENT extends InstrumentTemplate,
    INSTRUMENT_LINE extends InstrumentLineTemplate>
    implements InstrumentReaderTemplate<INSTRUMENT_LINE>,
    ...
    public PagedItem<INSTRUMENT_LINE> readInstruments(long actor, PagedItemInfo pagedItemInfo) {
        ...
        PagedItem<AccessedInstrument<INSTRUMENT>> page =
            instrumentReaderHelper.readInstruments(actor, pageRequest, pagedItemInfo.toFilterQuery()).getPagedItem();
        return afterReadInstruments(actor, page);
    }


    protected Set<INSTRUMENT_LINE> afterReadInstruments(
        long actor,
        ImmutableSet<AccessedInstrument<INSTRUMENT>> instrumentTemplates) {

        return instrumentTemplates.stream()
            .map(instrumentReaderHelper.getTransformer()::apply)
            .collect(Collectors.toSet());
    }

    protected PagedItem<INSTRUMENT_LINE> afterReadInstruments(long actor,
                                                              PagedItem<AccessedInstrument<INSTRUMENT>> pagedItem) {
    ...
    public PagedItem<INSTRUMENT_LINE> readInstrumentsByLab(long actor, long lab, PagedItemInfo pagedItemInfo) {
        ...
        final PageRequest pageRequest = toPageRequest(InstrumentTemplate.class, pagedItemInfo);
        PagedItem<AccessedInstrument<INSTRUMENT>> page =
            instrumentReaderHelper.readInstrumentsByLab(
                actor,
                lab,
                pageRequest,
                pagedItemInfo.toFilterQuery()
            ).getPagedItem();

        return afterReadInstrumentsByLab(actor, lab, page);
    }

    protected Set<INSTRUMENT_LINE> afterReadInstrumentsByLab(long actor,
                                                             ImmutableSet<AccessedInstrument<INSTRUMENT>> instruments) {
        return instruments.stream()
            .map(instrumentReaderHelper.getTransformer()::apply)
            .collect(Collectors.toSet());
    }

    protected PagedItem<INSTRUMENT_LINE> afterReadInstrumentsByLab(
        long actor,
        long lab,
        PagedItem<AccessedInstrument<INSTRUMENT>> pagedItem) {
        return builder(pagedItem, instrumentReaderHelper.getTransformer()).transform();
    ...
    public SortedSet<InstrumentItem> readInstrumentsWhereUserIsOperator(long actor) {
        ...
        return instrumentReaderHelper.readOperatedInstruments(actor)
            .transform(input -> transformers.instrumentItemTransformer().apply(input.instrument))
            .toSortedSet(transformers.instrumentItemComparator());
    }

    @Override
    public Set<INSTRUMENT_LINE> readInstrumentsByLabAndStudyType(long actor, long lab, long studyType) {
        return instrumentReaderHelper.readInstrumentLinesByLabAndStudyType(actor, lab, studyType).transform().toSet();
    }
